...
/*
 * Compat shim: seq_put_hex_ll() was only added upstream around v4.20/5.0
 * as a fast-path helper for /proc/*/maps. This kernel base doesn't have
 * it, so provide an equivalent built on seq_printf() instead of touching
 * struct seq_file internals directly.
 */
static void seq_put_hex_ll(struct seq_file *m, const char *delimiter,
