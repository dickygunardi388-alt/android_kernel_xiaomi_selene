...
/*
 * Compat shim: this kernel base still uses the old IDA API
 * (ida_pre_get()/ida_get_new_above()) and only has ida_simple_get()/
 * ida_simple_remove(), not the newer ida_alloc_min()/ida_free() (added
 * upstream in v5.0). The SUSFS patch above assumes the newer API, so
 * provide thin wrappers here instead of rewriting every call site.
 */
static inline int ida_alloc_min(struct ida *ida, unsigned int min, gfp_t gfp_mask)
{
	return ida_simple_get(ida, min, 0, gfp_mask);
}
